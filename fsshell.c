...
int cmd_cp (int argcnt, char *argvec[])
	...
	if(fs_isDir(dest) == 1) 
	{
		char src2[MAX_PATH_NAME];
		strcpy(src2, src);
		getFileName(src2);
		//if dest is a directory, append src filename to dest
		//first, check if there is a slash at the end of dest
		if(dest[strlen(dest)-1] != '/')
		{
			strcat(dest, "/");
		}
		//now append src filename to dest
		strcat(dest, src2);
	} 
...
int cmd_mv (int argcnt, char *argvec[])
	...
	// Check for correct number of arguments : 2, source and destination
	if (argcnt != 3)
		{
	...
	src = argvec[1];
	dest = argvec[2];
	//make sure src is not a directory

	switch(fs_isFile(src)) 
	{
		case -1:
	...
	if(compareFileLocation(src, dest) == 1)
	{
		return 0;
	}
	...
	b_close(testfs_src_fd);
	b_close(testfs_dest_fd);
	fs_delete(src); 		//delete the original file
#endif
	return 0;
	}
